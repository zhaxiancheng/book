...
<!--
	...
	function topage(pagenum){
		var form = document.forms["bookquery"];
...
<body class="ProducTypeHome2" onload="javascript:pageInit()">
    ...
    <div id="position">您现在的位置: <a href="/" name="linkHome">同大图书借阅管理</a> &gt;&gt; <em>图书查询结果</em> （${pageView.totalrecord}个）
	</div>
...
<c:forEach items="${pageView.records}" var="entry">	
		<div class="goodslist">
          <div class="goods" style="cursor:hand;background:url(<c:forEach items="${entry.styles}" var="pic">${pic.image140FullPath}</c:forEach>) center center no-repeat"><a href="<html:rewrite action="/book/view"/>?bookid=${entry.bookid}" target="_blank" title="${entry.name}" target="_blank">
            <img src="/images/global/book_blank.gif"  width="140" height="168"  border="0"/></a></div>
          <div class="goods_right">
                <h2><a href="<html:rewrite action="/book/view"/>?bookid=${entry.bookid}" target="_blank" title="${entry.name}" target="_blank" title="${entry.name}">图书名称： ${entry.name}</a></h2>
	           <div class="message"><ul>
			  <c:if test="${!empty entry.type}"> <li>类别：${entry.type.name}</li></c:if>
			   </ul></div>
	           <div class="content"><em>图书描述：</em><c:out value="${fn:substring(entry.description,0,200)}" escapeXml="false"/></div>
